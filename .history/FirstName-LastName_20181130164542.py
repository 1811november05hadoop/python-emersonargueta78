...
def rotateWord(words, key):
    ...
    for i in (0,len(characters)):
        ...
        isAlphabetic = characters[i].isAlphabetic();
        Character.isAlphabetic(characters[i]);
        isUpperCase = Character.isUpperCase(characters[i]);
        if(isUpperCase){
            if(!isAlphabetic){
                continue;
            }
            else if((characters[i] + key) > 'Z'){
                characters[i] = (char)((characters[i] + key)%'Z'+('A'- 1));
            }
            else{
